...
export const changelogItems: ChangeEntry[] = [
  ...
  { date: "Jul 9", title: "Aurelia is a real app you install, and it keeps itself up to date", description: "Aurelia is now a proper desktop app you install on your own computer, and it quietly updates itself in the background so you never have to reinstall. Installing used to crawl for minutes; it is quick now, because the whole thing ships as one bundle instead of tens of thousands of tiny files. Signing in to your Claude and Google accounts is a one-click sign-in that opens your browser, you can drag the window from the top edge on every page, and it shows its version in the corner." },
]
